...
import { QualityStandard } from '../src/types/tcm-core'
...
function convertMeridians(meridians?: string[]): MeridianType[] {
  ...
  return meridians.map((m) => meridianMap[m]).filter((m): m is MeridianType => m !== undefined)
}
...
herbsData.forEach((herb, index) => {
  ...
  if (
    herb.compatibilityInfo?.commonCombinations &&
    herb.compatibilityInfo.commonCombinations.length > 0
  ) {
...
${Object.entries(indexData.counts)
  .map(([key, count]) => `- ${key}.json (${count} 条记录)`)
  .join('\n')}
- index.json (索引文件)
- relations.json (关系映射)
...
function validateData() {
  ...
  const missingFiles = requiredFiles.filter((file) => !fs.existsSync(path.join(OUTPUT_DIR, file)))
...
function validateMaterials(materials: Material[]): any {
  ...
  materials.forEach((material, index) => {
    ...
    if (!material.flavor || material.flavor.length === 0)
      errors.push(`Material[${material.id}]: 缺少味(flavor)`)
  ...
  formulas.forEach((formula, index) => {
    ...
    if (!formula.ingredients || formula.ingredients.length === 0) {
      ...
      formula.ingredients.forEach((ing, idx) => {
        if (!ing.ingredientId)
          errors.push(`GranuleFormula[${formula.id}].ingredients[${idx}]: 缺少颗粒ID`)
