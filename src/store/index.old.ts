...
import type { Herb, SearchFilters, UIState, TCMExpert } from '../types'
    ...
    (set, get) => ({
      ...
      filterHerbs: () => {
        ...
        if (searchFilters.meridian) {
          filtered = filtered.filter((herb) => herb.meridians?.includes(searchFilters.meridian))
        }

        // 产地过滤
        if (searchFilters.origin) {
          filtered = filtered.filter((herb) => herb.origin?.includes(searchFilters.origin))
        }
